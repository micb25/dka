...
def plot_risk_level(masterframe):
    ...
    masterframe.columns = masterframe.columns.str.replace("Transmission Risk Level", "Level")
    ax = masterframe.plot.bar(stacked=True, cmap='viridis')
    ax.set_title("Risiko-Level in Diagnoseschlüsseln", position=(0.58, 0.9),fontweight="bold")
        ...
        for file in f:
            if file.endswith(".dat"):
                ...
                if ( len(pd) == 1 ) and ( len(pd[0]) == 3):
                    ...
                    dataframe['time'] =  datetime.date.fromtimestamp(timestamp).isoformat()
