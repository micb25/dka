...
# stats for y
stats "<awk -F, '{if ( NR > 1 ) print $6}' ../data_CWA/diagnosis_keys_statistics.csv" using 1 nooutput
...
plot  \
  ...
  \
  "<awk -F, '{if ( NR>1) {a=$1;c=b;b=$6}}END{print a, b, b-c}' ../data_CWA/diagnosis_keys_statistics.csv" using 1:2:(sprintf("%i (%+i)", $2, $3)) with labels point ls 2 ps 0.0 center offset char  0.0, 0.75 tc ls 5 notitle
