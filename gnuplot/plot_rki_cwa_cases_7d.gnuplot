...
set output '../plot_rki_cwa_cases_7d.png'
...
# data
plot  \
  "<awk -F, 'BEGIN{for(i=0;i<7;i++){a[i]=0}} { if (NR > 1) {for(i=0; i<6; i++){a[i]=a[i+1]};a[6]=100*$4;b=0;for(i=0;i<7; i++){b=b+a[i]};b = b / 7; print $1,b}}' ../data_CWA/correlation_CWA_RKI.csv" using 1:2 with linespoints ls 8 notitle, \
  \
